...
void CPropertyGridCombo::AddString(CString strItem)
{
...
BOOL CPropertyGridCombo::Create(DWORD dwStyle, CRect& rc, CWnd* pParent, int nId)
{
	...
	BOOL ret = CWnd::CreateEx(0,AfxRegisterWndClass(CS_HREDRAW|CS_VREDRAW), L"", dwStyle | WS_POPUP, rc, pParent, nId);
...
void CPropertyGridCombo::OnPaint()
{
	...
	//for (vector<CString>::iterator it = m_Items.begin(); it != m_Items.end(); ++it)
	for (int j=0;j<m_Items.size();j++)
	{
		...
		dc.DrawText(m_Items[j], rcItem, DT_SINGLELINE|DT_VCENTER|DT_LEFT|DT_NOPREFIX);
		y += m_line_height;
		i++;
	}

...
LPCTSTR CScriptObject::GetErrorString(int &nLine)
{
	m_szError[0] = 0;
	if (m_pScript != NULL)
	{
		try{
			...
			if (pError != NULL)
			{
				_bstr_t desc  = _bstr_t("Error: ") + pError->GetDescription();

				nLine=pError->GetLine();
...
BOOL CScriptObject::Exist(LPCTSTR funcName)
{
	_bstr_t szFunc;
	szFunc=GetScriptFunction(funcName);
	return (szFunc.length()>0);
}

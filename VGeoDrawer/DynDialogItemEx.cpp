...
DLGITEMTEMPLATECONTROLS CDynDialogItemEx::GetClassTypeByName(CString lpszClassName)
{
...
UINT CDynDialogItemEx::InitDialogItem(CString lpszClassName,
                                      DWORD dwStyle,
                                      DWORD dwExtendedStyle,
                                      LPRECT pRect,
                                      LPCTSTR lpszCaption,
...
PBYTE CDynDialogItemEx::FillBufferWithItemTemplate(BYTE *pdest)
{
  ...
  *(WORD*)pdest = (USHORT)m_eTypeControl;    // atom value for the control
  ...
  nActualChars = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)m_strCaption.GetBuffer(m_strCaption.GetLength()), -1, pchCaption, nChars);

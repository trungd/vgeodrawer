...
BOOL BCMenu::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
	...
	ColorMap whiteToNo;
	Color clr;
	bmp.GetPixel(1,1,&clr);
	whiteToNo.newColor=Color(0,255,255,255);
	whiteToNo.oldColor.SetFromCOLORREF(m_bitmapBackground);
	imageAttr.SetRemapTable(1,&whiteToNo);
	Bitmap bmp2(bmp.GetWidth(),bmp.GetHeight(),PixelFormat32bppARGB);
	Graphics gr(&bmp2);
	gr.DrawImage(&bmp,Rect(0,0,bmp.GetWidth(),bmp.GetHeight()),0,0,(int)bmp.GetWidth(),(int)bmp.GetHeight(),UnitPixel,&imageAttr);
	bmp2.GetHICON(&hIcon);
	//////////////////////////////////////////////////////////////////////////

...
int CDynDialogEx::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if (m_pFont == NULL) {
    ...
    wcsncpy(LogFont.lfFaceName, _T("Tahoma"), LF_FACESIZE);
    LogFont.lfHeight = 8;
...
INT_PTR CDynDialogEx::DoModal() 
{
  ...
  CString str=LogFont.lfFaceName;
  int cWC = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)str.GetBuffer(str.GetLength()), -1, NULL, 0);
  int nFontNameLen = cWC + 1;
  WCHAR *szFontName = new WCHAR[nFontNameLen];
  // Copy the string
  MultiByteToWideChar(CP_ACP, 0, (LPCSTR)str.GetBuffer(str.GetLength()), -1, (LPWSTR) szFontName, cWC);
  szFontName[cWC] = 0;
  ...
  //Prework for setting caption in dialog...
   cWC = MultiByteToWideChar(CP_ACP, 0, (LPCSTR)m_strCaption.GetBuffer(m_strCaption.GetLength()), -1, NULL, 0);
   int szBoxLen = cWC + 1;
  WCHAR *szBoxCaption = new WCHAR[szBoxLen];
  //Copy the string
   MultiByteToWideChar(CP_ACP, 0, (LPCSTR)m_strCaption.GetBuffer(m_strCaption.GetLength()), -1, (LPWSTR) szBoxCaption, cWC);
   szBoxCaption[cWC] = 0;
...
UINT CDynDialogEx::AddDlgControl(CString lpszClassName,
                                 CString lpszCaption,
                                 DWORD dwStyle,
                                 DWORD dwExtendedStyle,
                                 LPRECT pRect /*= NULL*/,
                                           ...
                                           UINT nID /*= 0*/)
{
  ...
  CDynDialogItemEx *pDynDialogItemEx = new CDynDialogItemEx;
  if (pDynDialogItemEx != NULL) {
    nRet = pDynDialogItemEx->InitDialogItem((CString)lpszClassName, dwStyle, dwExtendedStyle, pRect, lpszCaption, nID, TRUE);
    m_arrDlgItemPtr.Add(pDynDialogItemEx);
...
void CDynDialogEx::SetWindowTitle(CString lpszCaption)
{
